...
public class BeanUtil {
	...
	/**
	 * 文件上传的表单填充，要求表单项的name值和实体属性名（成员方法截取后得到的）要一样
	...
	public static <T> T fillBean(List<FileItem> items,Class<T> clazz){
		...
		try{
			...
			for(FileItem item : items){
				//4.判断是不是文件
				if(item.isFormField()){//普通表单项，不是文件表单项
					//5.取出表单中的name属性取值
					String fieldName = item.getFieldName();
					//6.使用UTF-8字符集取出表单数据
					String fieldValue = item.getString("UTF-8");
					//7.创建属性描述器对象，内省（封装了反射，主要反射setter&getter）
					// PropertyDescriptor  属性描述器，封装了实体类中getter&setter
					PropertyDescriptor pd = new PropertyDescriptor(fieldName,clazz);
					//8.获取写方法(setXxxx方法）  这里的属性不是成员变量名，而是根据getXxx，除去前三个字符后首字母小写的值
					Method method = pd.getWriteMethod();
					// 获取getter(getXxxx)
					//pd.getReadMethod()
					//9.把数据填充到bean中，反射调用实体对象的setter为属性设置值
