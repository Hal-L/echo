...
同时从`day03-switch_循环语句`的课程资料中找到：`模块页面\user\role.jsp`，拷贝到工程`/WEB-INF/pages/system/user/`目录下
...
（1）从`day03-switch_循环语句`的课程资料中找到`模块页面/unauthorized.jsp`，拷贝到项目的`webapp`下即可
